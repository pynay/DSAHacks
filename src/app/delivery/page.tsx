...
import { MapPin, Package, Plane, Plus, TrendingUp } from "lucide-react";
...
export default function DeliveryPage() {
  ...
  // Predicted-need multiplier for a neighborhood at a horizon step (from the ML 311 forecast).
  const ratioAt = (id: string, s: number) => {
    if (s === 0 || !series?.hoods[id] || !series.hoods[id].last) return 1;
    return series.hoods[id].values[s - 1] / series.hoods[id].last;
  };

  // Base zones projected forward to the selected step.
  const predictedBase = useMemo(
    () => baseZones.map((z) => ({ ...z, need: Math.max(0, Math.round(z.need * ratioAt(z.id, step))) })),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [baseZones, series, step],
  );
  const zones = useMemo(() => [...predictedBase, ...custom], [predictedBase, custom]);

  // Need-weighted centroid ("where need is heading") for a step, and the full trail.
  const centroidAt = (s: number): [number, number] | null => {
    const zs = baseZones
      .map((z) => ({ lng: z.lng, lat: z.lat, need: z.need * ratioAt(z.id, s) }))
      .filter((z) => z.need > 0);
  ...
  // Fastest-rising neighborhood at the selected horizon = where to send the drone to check.
  const hotspot = useMemo(() => {
    if (step === 0 || !series) return null;
    let best: { zone: DeliveryZone; pct: number } | null = null;
    for (const z of baseZones) {
      const r = ratioAt(z.id, step);
  ...
  const totalKm = zones.reduce((s, z) => s + haversineKm(DEPOT, z), 0);
    ...
    <div className="flex h-[calc(100vh-7rem)] gap-4">
      <div className="relative flex-1 overflow-hidden rounded-xl border border-stone-200 shadow-sm">
        ...
        <div className="pointer-events-none absolute bottom-3 left-3 rounded-lg bg-white/90 px-3 py-2 text-xs text-stone-600 shadow">
          ...
          <div className="flex items-center gap-2"><span className="inline-block h-3 w-3 rounded-full bg-fuchsia-500" /> predicted need center</div>
          <div className="flex items-center gap-2"><span className="inline-block h-3 w-3 rounded-[3px] bg-yellow-600" /> depot</div>
          <div className="mt-1 text-stone-400">{step === 0 ? "current need · click to add a drop" : `forecast: ${horizonLabels[step]}`}</div>
        </div>
      </div>
      ...
      <aside className="flex w-80 shrink-0 flex-col rounded-xl border border-stone-200 bg-white shadow-sm">
        <div className="border-b border-stone-200 p-4">
          ...
          <p className="text-xs text-stone-500">
            Need-weighted drop targets from the data commons. Step the ML forecast to see where need
            is heading.
          </p>
          ...
          <div className="mt-3">
            <div className="mb-1 flex items-center gap-1 text-[11px] font-medium text-stone-500">
              <TrendingUp size={12} /> Predicted need horizon
            </div>
          ...
          <div className="mt-3 grid grid-cols-3 gap-2 text-center">
            ...
            <div className="rounded-lg bg-yellow-50 py-2">
              ...
              <div className="text-[11px] text-stone-500">{step === 0 ? "total need" : "pred. need"}</div>
            </div>
            <div className="rounded-lg bg-yellow-50 py-2">
          ...
          {hotspot && (
            <div className="mt-3 rounded-lg bg-orange-50 p-3">
              <div className="text-[11px] font-medium uppercase tracking-wide text-orange-800">Predicted rising need</div>
              ...
              <p className="mt-0.5 text-[11px] text-orange-700">
                The model expects need to shift here. Send a drone to verify on the ground before pre-positioning food.
              </p>
              <Link
                href="/drone"
